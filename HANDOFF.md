...
**Iteración 28 (HECHA 2026-06-26): 2 bases FFB + condiciones reales por juego + coach de frenada (kg de célula de carga).** Pedido de Patricio (6 puntos). Verificado con investigación adversarial (workflow `wf_b3401e5e-4a7`, 12 agentes: 1 investigador + 1 verificador escéptico por juego + 2 de load cell) + tsc 0 + validate-engine 0 + script headless 32/32. **NO se pudo verificar en el navegador en vivo**: el dev local ya no levanta porque desde la iter 27 `schema.prisma` usa `postgresql` y el `DATABASE_URL` local no es Postgres → `getCurrentUser` (RootLayout) tira `PrismaClientInitializationError` en TODA página (incluido el fallback `DEV_NO_AUTH`, que también llama a prisma). Es un hueco de entorno PREVIO, no de esta feature. Para levantar local: poner un `DATABASE_URL` Postgres local (no apuntar al Neon de prod).
- **(1) Solo 2 bases FFB.** `hardware/fanatec.ts`: `fanatecBases` 6 → **2** (CSL DD 5/8 Nm + ClubSport DD+ 18 Nm). Limpiados los 6 `perBase` de los `ffb.ts` (acc/lmu×2/ac_evo/ac_rally/ea_wrc) a solo esas 2 claves; `validate-engine` ahora deriva `BASE_IDS` de `fanatecBases` (no más set hardcodeado) → caza claves de bases fantasma. Prosa de SEN/FFS recortada a las 2 bases.
- **(2) Temp de pista solo donde el juego la expone.** Verificado: ACC y LMU la exponen (se mantienen); **AC EVO y AC Rally NO** (en AC Rally era contaminación de datos de otro juego) → se quitó `trackTemp` de sus `conditionFields` + se borraron sus reglas `track_temp_high/low`. EA WRC ya no la tenía.
- **(3) Agarre por juego.** Verificado: grip/track-state solo en circuito con goma evolutiva (ACC/LMU/AC EVO) — ya estaba así; rally (EA WRC/AC Rally) no tiene eje de grip (usa superficie). Sin cambios salvo confirmar la corrección.
- **(4) Combustible por juego.** Verificado: ajustable en ACC/LMU/AC EVO (litros); NO en EA WRC/AC Rally — ya estaba así.
- **(5) Clima por juego.** Nuevo `GameData.conditionOptions` (subconjunto de valores por campo, types.ts). ACC/LMU mantienen seco/húmedo/mojado; **AC EVO, EA WRC y AC Rally NO tienen estado intermedio "húmedo"** → `conditionOptions.weather = ["dry","wet"]` y se borraron sus reglas `weather_damp`. El generador resetea condiciones al cambiar de juego para no dejar "húmedo" huérfano.
- **(6) Fuerza de freno de célula de carga (kg) por juego/auto/etapa.** Nuevo módulo puro `lib/brake-coach.ts` (`resolveBrakeClass` + `brakeForceRec` + notas de afinado/pista/pedal) y componente `components/brake-coach.tsx`, montado en el ResultPanel (recibe game/car/track). Rangos consensuados de investigación: GT3 50-70, GT4/TCR 40-58, Hypercar/LMP 60-80, Fórmula 65-85, calle 25-45, rally asfalto 35-52, rally grava/nieve 25-42 kg. En rally la clase sale de la SUPERFICIE de la etapa; en circuito, de la categoría (resolver por keywords, road antes que hypercar). Honesto: es punto de partida + heurística de afinado (bloqueas→sube kg; no llegas al 100%→baja), no un dato inventado por pista; nota de tope del pedal CSL ~60 kg vs ClubSport V3 90 kg.
- **Commits (pusheados a `origin/main`):** `d6c7d07` (feature: 18 archivos — 6 puntos) + `aedc233` (fix i18n: voseo→tuteo en 2 strings: `components/track-coach.tsx` "Marca…aquí" + `data/lmu/parameters.ts` "(P1-P20, aquí P13)"). `current-user.ts` (bypass `DEV_NO_AUTH`) queda FUERA del repo como siempre (sigue `M` local). Memoria nueva: `local-dev-needs-postgres.md` (por qué el dev local no levanta + cómo restaurar preview). Deploy Vercel a `configuracion.patagoniasimracing.cl` disparado por el push.
...
- **AC Rally — etapas individuales.** Los nombres NO están en el setup .sav; se sacaron de los TOC `acr/Content/Paks/*.utoc` (texto plano, IoStore) → **50 variantes** (10 etapas base × cortes × 2 sentidos) en 5 rallies. +**Charbonnière** (Alsace) que faltaba en la app. 9 → **59 pistas**. + **Škoda Fabia RS Rally2** cargado como baseSetup VERIFICADO (valores reales del `CarSetupsDataSaveSlot.sav`). Método guardado en memoria `sim-setup-file-formats` para DLC futuro (re-correr el parser del .utoc).
- **`Track.region`** (campo opcional nuevo en `types.ts`): la etapa declara su rally → el selector agrupa con `<optgroup>` nativo y queda navegable con 284/59 entradas. Backward-compatible (las pistas sueltas no lo llevan).
- **Preguntas por DISCIPLINA.** El picker ya era data-driven (muestra solo síntomas con regla). Faltaba el TEXTO: header rally **"¿Qué te pasa en la etapa?"** (no "pista"); `kerb_instability` en rally = **"Inestable en saltos y baches"** (rally no tiene pianos); EA WRC ya omitía "Neumáticos fríos" (sin presión ajustable). Claves nuevas `gen.problemsRally` + `symptom.kerb_instability_rally` (ES/EN). El `reason` de la regla en AC Rally también dejó de decir "pianos".
- **Coaching por DISCIPLINA.** `coach.ts`: `drivingTips` era un mapa plano (jerga de circuito leakeaba a rally: "pianos", "porpoising aero", "out-lap"). Nuevo `drivingTipsRally` (override parcial: kerb/bouncing/tyres_cold) + helper `drivingTipFor(symptom, isRally)`; `<TrackCoach>` recibe `isRally` desde el generador. Un único punto de verdad por disciplina.
- **Fix layout FFB.** El panel de selección `lg:sticky lg:top-20` (más alto que el viewport → sticky inútil) empujaba su caja 80px y **solapaba la tarjeta FFB con los botones Generar/Reiniciar**. Quitado el sticky → gap correcto (+28px). Verificado por mediciones de bounding-rect en vivo (desktop y mobile).
- **Castellano NEUTRO (Chile).** Pedido de Patricio + **guardado en memoria permanente** (`spanish-neutral-chile.md`, type feedback, con tabla voseo→tuteo). Convertido en TODA la copy ES: `coach.ts` (~22 strings: tips, variantes rally, playbook, plan de práctica), `track-coach.tsx`, `acc/rules.ts`, `lmu/rules.ts`, comentario en `types.ts`. "Frená→Frena", "dejá→deja", "tenés→tienes", "Manejalo→Manéjalo", "ojo con→cuidado con", "de a poco→poco a poco"… `grep` de voseo en `src` = **0**. EN intacto. Los `reason` de los demás rules.ts ya eran neutros (1ª plural "ablandamos/subimos").
- **Verificado:** `tsc --noEmit` 0 · `validate-engine` 0 (ea_wrc **100 autos / 284 pistas**, ac_rally **15 / 59**) · en vivo los 5 juegos (rally vs circuito divergen, coaching sin "pianos", `voseoOnPage: none`, 0 errores de consola). **Commit `9de108b` con SOLO los 12 archivos de la feature; los 2 del bypass `DEV_NO_AUTH` (`current-user.ts` + `proxy.ts`) y `tools/` (datasets con rutas personales `C:/Users/ASUS/...`) quedaron FUERA del repo público a propósito.**
