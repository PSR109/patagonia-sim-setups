...
export function FfbPanel({ game }: { game: GameData }) {
  ...
  return (
    <section className="card reveal reveal-4 overflow-hidden p-0">
      ...
      <div className="p-5 lg:p-6">
        ...
        {!ffb ? (
          ...
          <div className="mt-5 flex flex-col gap-5">
            ...
            <div>
              <p className="mb-2 font-display text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-brand">
                  ...
                  return (
                    <details key={rec.paramId} className="group">
                      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 text-sm">
                        ...
                        <span className="flex items-center gap-2">
                          <span className="telemetry font-semibold text-brand">{value}</span>
                      ...
                      {def && (
                        <div className="flex flex-col gap-1.5 px-3 pb-3 text-xs leading-relaxed text-muted">
                          ...
                          {rec.note && <p className="text-faint">→ {localize(rec.note, locale)}</p>}
                        </div>
                      )}
                    </details>
                  );
                  ...
                  return (
                    <details key={s.id} className="group">
                      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2.5 text-sm">
                        ...
                        <span className="flex items-center gap-2">
                          <span className="telemetry font-semibold text-sky">{value}</span>
                          <span className="text-muted transition-transform group-open:rotate-180">⌄</span>
                        </span>
                      </summary>
                      <div className="flex flex-col gap-1.5 px-3 pb-3 text-xs leading-relaxed text-muted">
                        <p>{localize(s.whatItDoes, locale)}</p>
                        {s.note && <p className="text-faint">→ {localize(s.note, locale)}</p>}
                      </div>
                    </details>
                  );
                })}
